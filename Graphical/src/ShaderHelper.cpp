...
namespace Graphical
{
	std::string ShaderHelper::ParseShader(const std::string& shaderSourcePath)
	{
	...
	unsigned int ShaderHelper::CompileShader(unsigned int shaderType, const std::string& path)
	{
	...
	unsigned int ShaderHelper::CreateShaderProgram(unsigned int vertexShader, unsigned int fragmentShader)
	{
		unsigned int s;
		s = glCreateProgram();
	...
	void ShaderHelper::DeleteShaders(unsigned int& s)
	{
		glDeleteShader(s);
	}

	void ShaderHelper::ShaderSuccess(unsigned int shader)
	{
		int success;
		char infoLog[512];
